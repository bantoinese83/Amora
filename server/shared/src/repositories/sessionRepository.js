 ...
 */
function validateSessionData(transcript, duration, audioChunks) {
    // Validate transcript
 ...
 */
function generatePreview(transcript) {
...
class SessionRepository {
    ...
    async getAll(userId) {
        ...
        try {
            const rows = await executeQuery(`SELECT id, user_id, date, duration_seconds, preview, transcript, analysis, audio_chunks, created_at, updated_at
         FROM sessions
         WHERE user_id = $1
         ORDER BY date DESC
         LIMIT 1000`, [userId]);
            const mappedSessions = rows
                .map((row) => this.mapRowToSession(row))
    ...
    async save(userId, session) {
        ...
        const validation = validateSessionData(session.transcript, session.durationSeconds, session.audioChunks);
        if (!validation.isValid) {
            throw new Error(validation.error || 'Invalid session data');
        }
        // Generate preview if not provided or invalid
        const preview = session.preview && session.preview.length > 0
        ...
        try {
            const result = await executeQueryOne(`INSERT INTO sessions (user_id, date, duration_seconds, preview, transcript, analysis, audio_chunks)
    ...
    async update(userId, id, updates) {
        ...
        try {
            const result = await executeQueryOne(`UPDATE sessions
         SET ${updatesArray.join(', ')}
         WHERE user_id = $${paramIndex++} AND id = $${paramIndex++}
         RETURNING id, user_id, date, duration_seconds, preview, transcript, analysis, audio_chunks, created_at, updated_at`, params);
    ...
    async getById(userId, id) {
        ...
        try {
            const result = await executeQueryOne(`SELECT id, user_id, date, duration_seconds, preview, transcript, analysis, audio_chunks, created_at, updated_at
         FROM sessions
     ...
     */
    async createFromTranscript(userId, transcript, duration, audioChunks) {
        if (!userId) {
            return null;
        }
        try {
