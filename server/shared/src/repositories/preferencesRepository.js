...
class PreferencesRepository {
    ...
    async getSelectedVoice(userId) {
        ...
        try {
            const result = await executeQueryOne(`SELECT selected_voice
    ...
    async setSelectedVoice(userId, voice) {
        ...
        try {
         ...
         VALUES ($1, $2)
         ON CONFLICT (user_id)
         DO UPDATE SET selected_voice = $2`, [userId, validatedVoice]);
        }
        catch (error) {
    ...
    async initializePreferences(userId) {
        ...
        try {
            await executeQuery(`INSERT INTO user_preferences (user_id, selected_voice)
         VALUES ($1, $2)
         ON CONFLICT (user_id) DO NOTHING`, [userId, 'Kore']);
        }
        catch (error) {
