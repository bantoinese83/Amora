...
router.post('/create-portal-session', async (req: Request, res: Response) => {
  try {
    const { returnUrl, customerId, customerEmail, userId } =
      req.body as StripePortalSessionParams & {
        userId?: string;
      };
    ...
    if (userId) {
      try {
        ...
        logger.warn(
          'Failed to get user Stripe customer ID',
          { userId },
    ...
    if (!customer) {
      logger.error('Customer not found for portal session', {
        userId,
        customerId,
        customerEmail,
        stripeCustomerId,
        ...
        try {
          ...
          if (user?.is_premium && !user.stripe_customer_id) {
            return res.status(400).json({
              error:
                'No active subscription found. Please subscribe to Amora Premium to manage your subscription.',
              needsSubscription: true,
            });
          }
        } catch (error) {
          logger.warn(
            'Failed to check user premium status',
            { userId },
            error instanceof Error ? error : undefined
          );
        }
      ...
      return res.status(404).json({
        error:
          'Customer not found. Please ensure you have an active subscription. If you recently subscribed, please wait a moment and try again.',
      });
    }
    ...
    const { userId: reqUserId, customerEmail: reqCustomerEmail } =
      req.body as StripePortalSessionParams & { userId?: string };
    ...
    logger.error(
      ...
      { userId: reqUserId, customerEmail: reqCustomerEmail },
      error instanceof Error ? error : undefined
    );
    if (error instanceof Error && error.message.includes('Stripe API key')) {
      res
        .status(500)
        .json({ error: 'Subscription management is not configured. Please contact support.' });
    } else {
      res
        .status(500)
        .json({ error: "We couldn't open subscription management. Please try again." });
    }
  }
});
