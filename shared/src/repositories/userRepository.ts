...
class UserRepository {
   ...
   */
  async createUser(email: string, pin: string, name: string): Promise<User> {
    // Validate inputs
   ...
   */
  async updatePremiumStatus(userId: string, isPremium: boolean): Promise<void> {
    if (!userId) {
      throw new Error('User ID is required');
    }
  ...
  async emailExists(email: string): Promise<boolean> {
    try {
      const normalizedEmail = validateEmail(email);
      const result = await executeQueryOne<{ exists: boolean }>(
