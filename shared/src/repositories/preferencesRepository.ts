...
class PreferencesRepository {
   ...
   */
  async getSelectedVoice(userId: string): Promise<string> {
    if (!userId) {
      return 'Kore';
    }
    ...
    try {
      const result = await executeQueryOne<{ selected_voice: string }>(
   ...
   */
  async setSelectedVoice(userId: string, voice: string): Promise<void> {
    if (!userId) {
